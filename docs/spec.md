...
| status | LogStatus | ✓ | `draft` \| `published` (기본값 `published`) |
...
| 값 | 설명 |
|----|------|
| `draft` | 작성 중 임시 저장 상태. 필수 필드를 모두 채우지 않아도 저장 가능. 통계/인사이트 집계 대상에서 제외 |
| `published` | 정식 발행 상태. 모든 필수 필드가 채워져야 함 |
...
- 폼 제출 버튼은 두 종류: "기록 추가/변경 저장"(`published`) + "임시 저장"(`draft`). 자세한 동작은 6.5 참조
...
- 모달은 `published` 로그만 노출한다. 드래프트는 미완성 레시피이므로 참고 가치가 낮고, 자동완성/통계와 동일한 격리 정책을 따른다.
- 백엔드 변경 없음 (기존 로그 조회 API 사용)

### 6.5 드래프트 저장

로그를 완성하지 않은 상태에서도 임시 저장(`status = 'draft'`)이 가능하다. 입력 허들을 낮추기 위한 기능.

**드래프트 저장 조건 (필수):**

`log_type` + 아래 중 하나 이상이 채워져 있어야 한다.

| log_type | 최소 필수 필드 (둘 중 하나) |
|----------|--------------------------|
| `cafe` | `cafe.cafe_name` 또는 `cafe.coffee_name` |
| `brew` | `brew.bean_name` 또는 `brew.brew_method` |

위 조건을 만족하지 못하면 드래프트 저장도 거부한다(`400 ValidationError`).

**published 저장 조건:**

기존 필수 필드(6.2 참조)를 모두 만족해야 한다. 드래프트와 동일한 API에서 `status` 필드만 다르게 보낸다.

**진입점 / 흐름:**

| 흐름 | 트리거 | 결과 |
|------|--------|------|
| 신규 드래프트 저장 | 작성 폼에서 "임시 저장" 버튼 | `status = 'draft'`로 신규 로그 생성, 목록 드래프트 섹션에 노출 |
| 드래프트 이어쓰기 | 목록의 드래프트 카드 탭 | 수정 폼이 이전 입력값으로 채워진 상태로 열림 |
| 드래프트 → 발행 | 수정 폼에서 "변경 저장" 버튼 | `status = 'published'`로 전환 |
| 드래프트 임시저장 갱신 | 수정 폼에서 "임시 저장" 버튼 | `status` 그대로 유지(`draft`), 입력값만 갱신 |
| 드래프트 삭제 | 일반 로그와 동일한 삭제 흐름 | 레코드 제거 |
...
|------|----|:----:|------|
| `draft` | `draft` | ✓ | 임시저장 갱신 |
| `draft` | `published` | ✓ | 발행 시 published 전체 필수 필드 검증 |
| `published` | `published` | ✓ | 일반 수정 |
| `published` | `draft` | ✗ | 발행된 로그는 다시 드래프트로 되돌릴 수 없다 |

**목록 표시:**

- 드래프트 로그는 목록 상단에 별도 섹션으로 묶어 표시한다(섹션 헤더: "작성 중인 기록").
- 시각적 구분: 점선 테두리 또는 "작성 중" 뱃지를 카드에 표기한다.
- 드래프트 카드 탭 → 수정 폼으로 이동.
- 드래프트 카드의 미입력 영역(예: 별점, 메모)은 "—" 또는 placeholder 텍스트로 빈 값을 노출한다.

**필터 동작:**

- 기본 목록 조회는 `published`만 반환한다(통계/인사이트 영향 차단).
- 별도 파라미터(`status=draft`)로 드래프트만 조회하거나, `status=all`로 전체 조회한다.
- 홈 화면은 `published` 목록과 `draft` 목록을 별도로 호출하여 두 섹션을 함께 그린다.

**집계/인사이트 제외:**

- 드래프트 로그는 통계, 자동완성 제안(`tasting_tags`, `companions`), 프리셋 사용 빈도 집계 등 모든 파생 데이터에서 제외한다.

**고려사항:**

- 드래프트 방치(예: 30일 경과)에 대한 알림/자동 삭제 정책은 추후 결정. 현재는 무한 보존.
- 복제(`cloneToFormState`) / 프리셋 / 레시피 불러오기로 채워진 폼도 동일한 흐름으로 드래프트 저장이 가능하다.
...
*Last updated: 2026-05-10 (v0.11 — 드래프트 저장 기능 추가, status 필드 도입)*
