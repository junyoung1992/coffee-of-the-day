...
### [DEBT-8] 드래프트 폼의 brew_method 기본값 모호성
- **상태:** [ ]
- **우선순위:** P3
- **맥락:** `createEmptyFormState()`가 `brew.brewMethod`를 `'pour_over'`로 기본 설정하기 때문에, 사용자가 명시적으로 추출 방식을 선택했는지 vs 기본값을 그대로 둔 것인지 구분할 수 없다. 결과적으로 `canSaveAsDraft()`는 brew 폼에서 사실상 항상 `true`를 반환하며, 빈 brew 폼도 임시 저장 가능 상태가 된다. Issue #9 구현 시 알려진 한계로 받아들였다.
- **할 일:** UX 방향을 먼저 논의한다. 선택지: (1) `brewMethod` 기본값을 빈 값으로 두고 사용자가 명시 선택하도록 강제, (2) 별도 `userSelectedBrewMethod: boolean` 플래그로 의도 추적, (3) 현재 상태 유지하고 brew 빈 폼은 published 검증에 맡김. 방향 결정 후 구현한다.
- **완료 기준:** 구현 방식 결정 후 별도 정의.
- **참조:** `docs/issues/9-draft-save/plan.md` (위험 요소 #2), `frontend/src/pages/logFormState.ts:canSaveAsDraft`

---

### [DEBT-9] 드래프트 카드 접근성 개선 — 점선/뱃지의 스크린 리더 노출
- **상태:** [ ]
- **우선순위:** P3
- **맥락:** `LogCard.tsx`에서 드래프트 카드를 점선 테두리 + "작성 중" 텍스트 뱃지로 구분한다. 텍스트 뱃지는 스크린 리더에 노출되지만, 점선 테두리는 시각 정보일 뿐이다. 또한 드래프트 카드는 `/logs/{id}/edit`로 이동하지만 published 카드와 시각/문구만 다르고 ARIA 속성으로 구분되지 않는다.
- **할 일:** 카드에 `aria-label`을 부여하여 "작성 중인 카페 기록 — 블루보틀, 메뉴 미작성" 형태의 컨텍스트를 스크린 리더에 전달한다. 키보드 탐색으로 published 카드와 draft 카드를 구분 인식할 수 있는지 검증한다.
- **완료 기준:**
  - 드래프트 카드에 적절한 `aria-label` 부여
  - 시맨틱 마크업으로 "작성 중" 상태가 보조 기술에서도 확인 가능
- **참조:** `docs/issues/9-draft-save/plan.md` (UX 디테일)

---

### [DEBT-10] 드래프트 자동 만료/알림 정책
- **상태:** [ ]
- **우선순위:** P3
- **맥락:** Issue #9에서는 드래프트가 오래 방치되었을 때의 처리 정책을 미정으로 두고 무한 보존을 채택했다. 시간이 지나면 잊혀진 드래프트가 누적되어 홈 화면 상단을 차지할 수 있다.
- **할 일:** 사용자와 정책 방향을 논의한다. 선택지: (1) N일 경과 시 자동 삭제, (2) 일정 기간 후 알림 표시, (3) 별도 "오래된 드래프트" 섹션으로 이동. 방향 결정 후 백엔드 배치/스케줄러 또는 프론트엔드 표시 로직을 구현한다.
- **완료 기준:** 구현 방식 결정 후 별도 정의.
- **주의:** `docs/spec.md` 6.5 갱신 필요.
- **참조:** `docs/issues/9-draft-save/plan.md` (UX 디테일 — "드래프트 방치"), `docs/spec.md` 6.5 (고려사항)
