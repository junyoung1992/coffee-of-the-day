...
- [x] **`buildPresetBody`에서 brew `recipe_detail` 누락**
...
- [x] **`EditModal.buildUpdateBody`에서 brew `recipe_detail` 누락**
...
- [x] **`handleSavePreset` 후 mutation 상태 미리셋**
  - Target: `frontend/src/pages/LogDetailPage.tsx`
...
- [x] **`useUpdatePreset` 중복 invalidation 제거**
